...
| 🔴 **Alta** | **El chip de precisar, en la pantalla.** El SQL, la RPC (`precisar_tecnica`) y el helper del cliente (`precisar()` en `db.ts`) están hechos y probados en `bjj_29`; falta el paso por el que se toca: al cerrar el roll y desde el historial, solo si la técnica tiene variantes, nunca bloqueando. Sin esto la fase 2 de las mecánicas no se usa. |
