...
| ~~🔴 Alta~~ | ~~**La cola no puede perder nada**~~ — **hecho**: espera creciente global (1s/2s/5s/15s/60s/5min), 4xx separado de 5xx, nada se descarta nunca solo, y la píldora distingue "sin subir" de "con error" con detalle tocable y aviso al cerrar. `pruebas/cola.js`, 16 comprobaciones. |
| 🔴 **Alta** | **Recolector de errores** (Sentry gratuito). Hoy te enteras de los fallos si alguien te los cuenta. Estás operando a ciegas. **Bloqueado por Felipe: hace falta una cuenta y una clave.** |
...
| ~~🔴 Alta~~ | ~~**Generar los tipos desde la base**~~ — **hecho**: `scripts/generar-tipos.py` vuelca el esquema a `esquema.generado.ts` y `database.types.ts` comprueba contra él en tiempo de compilación. No se regenera el fichero de mano: lleva dentro el porqué de cada decisión. Probado viéndolo fallar. |
| ~~🔴 Alta~~ | ~~**`v_tecnicas_practicante` con `modalidad` y `fecha`**~~ — **hecho** (`bjj_30`): `analisis()` lee de la vista en vez de recalcular. Comprobado que el `tec` da el mismo md5 antes y después en los tres filtros. |
| ~~🟡 Media~~ | ~~**Sellar los eventos espejo**~~ — **hecho** (`bjj_31`): `eventos.par_evento_id`, generado al registrar y copiado al espejar, y `precisar_tecnica()` actualiza los dos. Rellenado hacia atrás emparejando por posición: 1424 eventos, 612 pares. |
...
| ~~🔴 Alta~~ | ~~**El modo observador sigue colapsando los dos Open Mats**~~ — **hecho** (`bjj_35`): `p_quedada` viaja en `registrar_roll_observado`, con `default null` para que lo ya encolado siga entrando. Se **reemplazó** la firma en vez de sobrecargarla —dos `p_par` vivas serían ambiguas para PostgREST— y el puente de `p_grupo` sigue resolviendo, medido en producción. `espejar_roll` no cambió: lee la quedada de la sesión del original. |
| 🟡 Media | **El ranking CONGELADO corta con `having count(*) >= 2`.** La tabla en vivo ya enseña a todos, pero el informe guardado sigue dejando fuera a quien hizo un solo roll. Decidir si esa regla tiene sentido ahora que la tabla no premia: si se quita, hay que regenerar informes, y eso es otra conversación. |
| 🟢 Baja | **Dos sesiones vacías de Felipe colgando de «Battle for Namek»**, las dos con cero rolls. Son las que hacen que `asistentes` diga 3. No molestan, pero ensucian el recuento congelado de ese informe. |
| 🔴 **Alta** | **La app calcula el día en UTC y Postgres en local.** `new Date().toISOString().slice(0,10)` en `entreno` da la fecha UTC; entre las 00:00 y las 02:00 en España es la de ayer. Consecuencia medida: un roll observado de un Open Mat se **rechaza** con `check_violation` por la guarda de fecha de `bjj_35`, y `v_mi_quedada_hoy` no trae el Open Mat, así que ni sale el chip. Cae en la cola como «necesita atención», no se pierde. El arreglo es una función `hoy()` que use la fecha local, y toca el camino de registro: no se hace la víspera de un Open Mat. |
| 🟡 Media | **Reconstruir los 59 pares huérfanos anteriores al 2026-08-02.** Se puede: el `par_id` está y los eventos están, así que `espejar_roll()` los completa uno a uno. Hay que decidir si se hace, porque cambia el histórico de gente — y con ello sus logros y sus informes ya congelados. |
| 🔴 **Alta** | **El feed está CAÍDO en producción, no lento.** `feed()` tarda **10 547 ms** contra el `statement_timeout` de 8 s del rol `authenticated`: falla siempre con `57014`. Paginar primero y enriquecer después **no lo salva** —el esqueleto de las 8 ramas sin `datos` ya son 6 435 ms— porque el suelo son tres vistas de logros que recalculan el histórico entero (3 171 / 3 203 / 3 240 ms) y **no bajan al filtrar por id**. Queda la tabla precalculada con su refresco. Medir siempre con `set local role authenticated` y un `request.jwt.claims` de verdad: como propietario tarda 14 ms y parece perfecto. |
| 🔴 **Alta** | **`puntos_roll()` se llama por cada roll del histórico dentro de las vistas de logros.** Lo señala el `CONTEXT` del 57014. Es el suelo de los 3,2 s, así que puede que arreglarlo aquí valga por la tabla precalculada entera — mirarlo antes de construir la tabla, no después. |
| 🟡 Media | **Los cinco contactos sin cuenta tienen «guardarle sus rolls» apagado** (Goku, Vegeta, Freezer, Krilin, Piccolo). Mientras siga así, de sus rolls observados se guarda un solo lado y no salen en el informe. Se toca desde la ficha; no hace falta migración. |
| 🟢 Baja | **`analisis.js` y la semilla ya no cuadran en números.** El recorrido espera 176 rolls / 44 sesiones y la semilla —que llevaba rota desde `bjj_27` y ahora sí corre— produce otra cosa. Hay que decidir cuál manda: si el recorrido lee las cuentas de la base o si la semilla se fija. |
| 🔴 **Alta** | **`supabase_admin` le regala a `anon` todo objeto nuevo de `public`** (36 entradas: tablas, secuencias y funciones). `postgres` no puede cerrarlo: **hay que pegarlo en el panel**, y el SQL está en la cabecera de `db/32`. Después, reaplicar ese fichero — pasa en silencio si quedó cerrado. Hoy no es brecha (nadie crea tablas como ese rol), es una trampa para el futuro: una tabla sin `enable row level security` nacería legible por cualquiera con la clave pública. |
| 🟡 Media | **`enganchar_del_dia()` es demasiado ancha con dos Open Mats.** Agarra todas tus sesiones de esa fecha que no sean ya de ese Open Mat, así que arreglar el de la tarde se lleva los rolls de la mañana. Por eso salió de Entreno. En Quedadas sigue siendo lo correcto para el día que se te olvidó, pero debería preguntar cuáles. |
| 🟢 Baja | **«Suelto» no abre sesión nueva si ya hay una del día.** `p_quedada` nulo significa "me da igual la quedada" (bjj_34, a propósito), así que un roll marcado «Suelto» después de observar en un Open Mat acaba contando en él. Haría falta distinguir "no me importa" de "explícitamente ninguno", y eso es un tercer estado. |
...
| 🟡 Media | **Dos baterías se saltan enteras en CI y llevan tiempo en verde sin comprobar nada.** `enganchar-quedada.sql` y `sesion-por-quedada.sql` empiezan buscando practicantes con `user_id`, y en CI la base nace vacía: avisan y salen. `observador-quedada.sql` (`bjj_35`) se monta sus propias cuentas dentro de la transacción; hay que hacer lo mismo con las otras dos. Un aviso que nadie lee no es una red. |
| 🟢 Baja | **`logros.js` falla el día 1 de cada mes.** El ranking es del mes en curso y la semilla no garantiza logros ahí. No es regresión: falla igual sin ninguna migración nueva. El arreglo va en la semilla —que ponga logros del mes en curso—, no en aflojar la comprobación. |
