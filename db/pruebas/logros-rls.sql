-- ============================================================
--  LOGROS · que la RLS por grupo tambien los tape
--
...
--  que sí las tienen (bjj_15, lectura por grupo). Eso DEBERIA bastar, pero
...
--  conviertan en una puerta lateral que enseña la actividad de otro grupo.
--
...
begin
  -- Alguien de OTRO grupo, con un logro suyo para que la prueba distinga
  -- "no ve nada" de "no ve lo de los demas".
  delete from practicantes where academia = 'PRUEBAS-RLS';
  delete from grupos where slug = 'pruebas-rls';
  ...
  insert into grupos (nombre, slug, codigo_union)
  ...
  insert into sesiones (practicante_id, fecha, modalidad, tipo, grupo_id)
  values (v_fuera, current_date, 'gi', 'sparring', v_otro_grupo) returning id into v_s;
  insert into rolls (id, sesion_id, orden, origen, resultado, posicion_inicio, rol_inicio)
  ...
  if v_ve_ajenos <> 0 then
    raise exception
      'FALLO GRAVE: desde otro grupo se ven % logros ajenos. Los logros son '
      'una puerta lateral a la actividad de otra academia.', v_ve_ajenos;
  end if;
  raise notice 'PASS  desde otro grupo, los logros ajenos devuelven cero filas';
  ...
  -- El ranking del mes va por grupo y no puede colar a nadie de fuera.
  if exists (select 1 from v_logros_mes where practicante_id = v_dentro) then
    raise exception 'FALLO GRAVE: el ranking del mes enseña a gente de otro grupo';
  end if;
