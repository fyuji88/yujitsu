...
--
--    psql ... -f db/pruebas/grupos-rls.sql
--
...
-- ------------------------------------------------------------
-- Preparacion: dos cuentas dentro del grupo por defecto y una fuera.
--
...
delete from grupos       where id = 'eeeeeeee-0000-0000-0000-00000000000e';
...
-- a fichas del grupo que ya tienen historial detras.
delete from practicantes where user_id in (
  'aaaaaaaa-0000-0000-0000-00000000000a',
  'bbbbbbbb-0000-0000-0000-00000000000b',
  'cccccccc-0000-0000-0000-00000000000c');

-- Los dos practicantes con mas rolls del grupo por defecto hacen de "dentro".
with dentro as (
...
-- Y uno fuera: ficha nueva, grupo nuevo, con una sesion y un roll suyos.
...
insert into miembros_grupo (grupo_id, practicante_id, rol)
values ('eeeeeeee-0000-0000-0000-00000000000e',
        'dddddddd-0000-0000-0000-00000000000d', 'admin');

insert into sesiones (id, practicante_id, fecha, grupo_id)
...
insert into rolls (id, sesion_id, orden, resultado)
...
-- Cuantos rolls hay del grupo por defecto: es el numero que "dentro1" tiene
...
select count(*)::int as rolls
  ...
  join miembros_grupo m on m.practicante_id = s.practicante_id
 where m.grupo_id = (select grupo_id from miembros_grupo
...
\echo ''
\echo '=== 1. Ve lo suyo y lo de todo el grupo, con cuenta exacta'
begin;
  ...
  begin
    ...
    if v_ve <> v_esp then
      raise exception 'FALLO: ve % rolls y deberia ver % (los de su grupo)', v_ve, v_esp;
    end if;
    raise notice 'PASS  ve los % rolls del grupo, ni uno mas ni uno menos', v_ve;
  end $$;
rollback;


\echo ''
\echo '=== 2. NO ve nada de un practicante de otro grupo'
begin;
  select set_config('request.jwt.claims',
    '{"sub":"aaaaaaaa-0000-0000-0000-00000000000a"}', true);
  set local role authenticated;
  ...
  begin
    ...
    if n > 0 then raise exception 'FALLO GRAVE: ve % sesiones de otro grupo', n; end if;

    select count(*) into n from rolls
     where id = '99999999-0000-0000-0000-000000000009';
    if n > 0 then raise exception 'FALLO GRAVE: ve el roll de otro grupo'; end if;

    select count(*) into n from eventos
     where roll_id = '99999999-0000-0000-0000-000000000009';
    if n > 0 then raise exception 'FALLO GRAVE: ve % eventos de otro grupo', n; end if;

    raise notice 'PASS  de un practicante de otro grupo no ve ni sesiones, ni rolls, ni eventos';
  end $$;
rollback;
...
begin;
  ...
  begin
    select count(*) into n from rolls
     where id <> '99999999-0000-0000-0000-000000000009';
    if n > 0 then raise exception 'FALLO GRAVE: el de fuera ve % rolls del grupo', n; end if;
    select count(*) into n from rolls;
...
begin;
  ...
  begin
    ...
    begin
      ...
      raise exception 'FALLO GRAVE: ha insertado una sesion de un compañero de grupo';
...
do $$
declare v_total int; v_grupo int;
begin
  -- Como superusuario, todo lo que hay.
  select count(*) into v_total from rolls;
  -- Lo que queda dentro del grupo por defecto mas el roll del de fuera.
  select rolls into v_grupo from esperado;
  if v_grupo + 1 <> v_total then
    raise exception 'FALLO: hay % rolls en total y solo % quedan alcanzables (%. sin dueño?)',
      v_total, v_grupo + 1, v_total - v_grupo - 1;
  end if;
  raise notice 'PASS  los % rolls siguen alcanzables: % del grupo y 1 del de fuera',
    v_total, v_grupo;
end $$;
